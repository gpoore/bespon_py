...
\newbool{grammar@instring}
\makeatletter
\begingroup\catcode`\~=12\lccode`\~=`\\\lowercase{\endgroup\def\grammar@backslashchar{~}}
\def\grammar@process@string#1{%
  \ifx#1\grammar@undefined\relax
    \let\grammar@process@string@next\relax
  \else
    \expandafter\ifx\grammar@backslashchar#1\relax
      \let\grammar@process@string@next\grammar@string@backslash
    \else
      #1%
      \let\grammar@process@string@next\grammar@process@string
    \fi
  \fi
  \grammar@process@string@next
}
\def\grammar@string@backslash#1{%
  \ifx#1x\relax
    \let\grammar@process@string@next\grammar@string@x
  \else
    \ifx#1u\relax
      \let\grammar@process@string@next\grammar@string@u
    \else
      \ifx#1U\relax
        \let\grammar@process@string@next\grammar@string@U
      \else
        \def\grammar@process@string@tmp{\detokenize{#1}}%
        \let\grammar@process@string@next\grammar@string@other
      \fi
    \fi
  \fi
  \grammar@process@string@next
}
\def\grammar@string@x#1#2{%
  \textcolor{MediumSlateBlue}{\textbackslash x}%
  \textcolor{blue}{#1#2}%
  \grammar@process@string}
\edef\grammar@charlb{\iftrue\string{\else}\fi}
\edef\grammar@charrb{\iffalse{\else\string}\fi}
\def\grammar@string@u{%
  \expandafter\@ifnextchar\grammar@charlb{\grammar@string@u@i}{\grammar@string@u@ii}}
\expandafter\expandafter\expandafter\def\expandafter\expandafter\expandafter\grammar@string@u@i\expandafter\grammar@charlb\expandafter#\expandafter1\grammar@charrb{%
  \textcolor{MediumSlateBlue}{\textbackslash u}%
  \textcolor{blue}{\{#1\}}%
  \grammar@process@string}
\def\grammar@string@u@ii#1#2#3#4{%
  \textcolor{MediumSlateBlue}{\textbackslash u}%
  \textcolor{blue}{#1#2#3#4}%
  \grammar@process@string}
\def\grammar@string@U#1#2#3#4#5#6#7#8{%
  \textcolor{MediumSlateBlue}{\textbackslash U}%
  \textcolor{blue}{#1#2#3#4#5#6#7#8}%
  \grammar@process@string}
\def\grammar@string@other{%
  \textcolor{blue}{\textbackslash\grammar@process@string@tmp}\grammar@process@string}
\expandafter\expandafter\expandafter\def\expandafter\expandafter\expandafter\grammar@string@repeat\expandafter\grammar@charlb\expandafter#\expandafter1\grammar@charrb{%
  \textcolor{DarkRed}{\{#1\}}}
\begingroup
\catcode`\'=\active
\catcode`\"=\active
...
\gdef\grammar@activate{%
  \expandafter\def\expandafter\@noligs\expandafter{%
    ...
    \def'####1'{%
      \begingroup
      \booltrue{grammar@instring}%
      \textcolor{DarkBlue}{%  
        \textquotesingle
        \def"{{\detokenize{"}}}%
        \grammar@process@string####1\grammar@undefined%
        \textquotesingle}%
      \boolfalse{grammar@instring}%
      \endgroup
      \expandafter\@ifnextchar\grammar@charlb{\grammar@string@repeat}{}}%
    \def"####1"{%
      \begingroup
      \booltrue{grammar@instring}%
      \textcolor{DarkBlue}{%
...
\newenvironment{grammarbox}[1]%
 {\begin{tcolorbox}[colbacktitle=LightGreen,
                    coltitle=black,
                    title=#1]}
 {\end{tcolorbox}}
...
When an implementation receives a binary string or stream, it must decode the string or stream as UTF-8 unless an encoding is specified.

Regardless of whether an implementation receives a Unicode or binary string or stream, it must check the start of the string or stream for a single Unicode byte order mark (BOM) U+FEFF, and discard it if present.


\subsubsection{Unicode validation}

The Unicode string or stream containing data must be validated according to the following procedure.  Implementations may choose to validate all at once before processing or on the fly during processing.  Implementations should provide an option for stricter validation, so that only code points that are appropriate for a given application can be allowed.  This is particularly important when security is a concern.  Implementations may also provide an option that allows a less strict validation by permitting a broader range of literal code points.  In both cases, the standard validation must be the default.
...
\paragraph{Disallowed code points}  The string or stream must be checked for code points that are not allowed to appear literally.  These code points must only be represented in escaped form.  The presence of any of these code points must result in an error.
\begin{itemize}
\item All code points with Unicode General\_Category value Cc (``Other, control'')\footnote{Cc includes the ranges U+0000 through U+001F, and U+007F through U+009F (\href{http://www.unicode.org/versions/Unicode9.0.0/ch02.pdf}{The Unicode Standard, Version 9.0, Chapter 2.4}).} are prohibited, with the exception of horizontal tab U+0009 (|\t|) and line feed U+000A (|\n|).  Note that any carriage returns U+000D (|\r|) not removed by newline normalization are not allowed.  These code points are prohibited because they may be invisible in some editors and because some control characters, such as the null byte, have security implications.

\item All 7 code points specified as newlines by Unicode\footnote{\href{http://www.unicode.org/versions/Unicode9.0.0/ch05.pdf}{The Unicode Standard, Version 9.0, Chapter 5.8}.} are prohibited, with the exception of line feed U+000A (|\n|).  This includes vertical tab U+000B (|\v|), form feed U+000C (|\f|), carriage return U+000D (|\r|), and next line U+0085 (NEL), which are already covered under the Cc category.  It also includes line separator U+2028 and paragraph separator U+2029.  These characters are prohibited to ensure that newline treatment is uniform and to guarantee that the use of any newlines beyond line feed U+000A (|\n|) is explicit.

\item All 12 code points with Unicode property \href{http://unicode.org/reports/tr9/}{Bidi\_Control} are prohibited: U+061C, U+200E through U+200F, U+202A through U+202E, and U+2066 through U+2069.
...
The form of a validated Unicode string or stream after the removal of a single, leading, optional BOM U+FEFF and after newline normalization (|\r\n| $\rightarrow$ |\n|) may be summarized as follows.  Note that this does \emph{not} describe valid \bespon\ data, merely the necessary form of a Unicode string or stream that \emph{may} contain valid \bespon\ data.  A validated Unicode string or stream is a necessary but not sufficient requirement.
\begin{grammarbox}{Validated Unicode string or stream}
\begin{grammar}
unicode_code_point = "\u0000".."\U0010FFFF"
unicode_category_cc = "\u0000".."\u001F" | "\u007F".."\u009F"
unicode_newline_codepoint = "\u000A".."\u000D" | "\u0085" | "\u2028" | "\u2029"
unicode_property_bidi_control = "\u061C" | "\u200E".."\u200F" | 
                                "\u202A".."\u202E" | "\u2066".."\u2069"
unicode_surrogate = "\uD800".."\uDFFF"
bom = "\uFEFF"
unicode_noncharacter = "\uFDD0".."\uFDEF" | "\uFFFE" | "\uFFFF" |
                       ...
                       "\U000FFFFE" | "\U000FFFFF" |
                       "\U0010FFFE" | "\U0010FFFF"
valid_code_point = unicode_code_point
                   - (unicode_category_cc - "\t" - "\n")
                   - (unicode_newline_codepoint - "\n")
                   - unicode_property_bidi_control
                   - unicode_surrogate
                   - bom
                   - unicode_noncharacter
validated_string_or_stream = valid_code_point+
\end{grammar}
\end{grammarbox}
...
\section{Base grammatical elements}
\subsection{Indentation and whitespace}
\begin{grammar}
space = "\x20"
spaces = space+
tab = "\x09" = "\t"
indent_code_point = space | tab
indent = indentation_code_point+
newline = "\x0A" = "\n"
whitespace = (indent_code_points | newline)+
unicode_whitespace = unicode_property_white_space
                   = "\u0009".."\u000D" | "\u0020" | "\u0085" | "\u00A0" | 
                     "\u1680" | "\u2000".."\u200A" | "\u2028" | "\u2029" |
                     "\u202F" | "\u205F" | "\u3000"
\end{grammar}
...
\begin{grammar}
lower_ascii_alpha = "a".."z"
upper_ascii_alpha = "A".."Z"
ascii_alpha = lower_ascii_alpha | upper_ascii_alpha
ascii_code_point = "\u0000".."\u007F"
\end{grammar}
...
\paragraph{Decimal}
\begin{grammar}
dec_integer = (sign indentation*)? (zero | nonzero_dec_digit ("_"? dec_digit)*) 
\end{grammar}

\paragraph{Hexadecimal}
\begin{grammar}
hex_integer = (sign indent*)? "0x" "_"?
              (lower_hex_digits_underscores | upper_hex_digits_underscores)
\end{grammar}

\paragraph{Octal}
\begin{grammar}
oct_integer = (sign indent*)? "0o" "_"? oct_digits_underscores
\end{grammar}

\paragraph{Binary}
\begin{grammar}
bin_integer = (sign indent*)? "0b" "_"? bin_digits_underscores
\end{grammar}

\paragraph{General integer}
\begin{grammar}
integer = dec_integer | hex_integer | oct_integer | bin_integer
\end{grammar}

Hex, octal, and binary integers must use lowercase |x|, |o|, and |b| in their base prefixes.  This prevents the confusable possibility of octal |0O|\vmeta{digits} while maintaining consistency across bases.  Hex integers may use either uppercase or lowercase letters, but must not mix cases.  Integers must not have leading zeros, to prevent confusion with languages that use a leading zero to indicate octal values.

Integer precision is not specified, but an absolute minimum of signed 32 bits is expected.  This allows the comfortable substitution of a 64-bit float in languages without an integer type.  Implementations must check for integer overflow when loading data, and raise an error if any integer cannot fit in the specified integer type.

\textcolor{red}{\bfseries In all grammar for line wrapping/unwrapping, need to find a way to express a pattern that is repeated exactly.  Need an indent variable that refers to any one specific sequence that meets the indent pattern, and that is repeated multiple times.  Basically, need a regex backreference.}

Note that the integer definitions above do not account for the potential of line wrapping occurring between the sign and the first digit; the definitions apply after any line unwrapping.  In indentation-based form, unwrapping may be summarized as follows.
\begin{grammar}
common_indent = indent*
wrapped_integer = common_indent preceding_content* (integer_sign) indent* "\n"
                  common_indent indent+ integer_unsigned_value
unwrapped_integer = integer_sign integer_unsigned_value
\end{grammar}
A line that continues an integer must be indented more than the previous line, with exactly the same common indentation.  Meanwhile, in inline form, the only requirement is that both lines share the minimum inline indentation.
\begin{grammar}
inline_indent = indent*
wrapped_integer = inline_indent indent* preceding_content* (integer_sign) indent* "\n"
                  inline_indent indent* integer_unsigned_value
\end{grammar}
Full details about line unwrapping algorithms are provided in \cref{sec:indentation}.

\subsubsection{Float (\texttt{float})}

Float literals may take decimal or hexadecimal forms.  Hex form is convenient for avoiding small rounding errors upon round tripping.  As with integers, single underscores are allowed between adjacent digits and after the hex base prefix.  Single underscores before the exponent are also allowed.  Also as with integers, the float definitions apply after any line unwrapping has been performed (\cref{sec:indentation}).

\paragraph{Decimal}
\begin{grammar}
dec_exponent = ("e" | "E") sign? dec_digits_underscores
dec_float = (sign indent*)? (zero | nonzero_dec_digit ("_"? digit)*)
            ("." dec_digits_underscores ("_"? dec_exponent)? | "_"? dec_exponent)
\end{grammar}

\paragraph{Hexadecimal}
\begin{grammar}
hex_exponent = ("p" | "P") sign? digit ("_"? digit)*
hex_float = (sign indent*)? "0x" "_"?
            (lower_hex_digits_underscores
                ("." lower_hex_digits_underscores ("_"? hex_exponent)? | 
                 "_"? hex_exponent) |
             upper_hex_digits_underscores
                ("." upper_hex_digits_underscores ("_"? hex_exponent)? |
                 "_"? hex_exponent))
\end{grammar}

\paragraph{Special values}
\begin{grammar}
infinity = (sign indent*)? "inf"
not_a_number = (sign indent*)? "nan"
\end{grammar}

\paragraph{General float}
\begin{grammar}
float = dec_float | hex_float | infinity | not_a_number
\end{grammar}


Leading zeros before the decimal point in decimal form and capital |X| in hexadecimal form are not allowed.  Both capital and lowercase |e| and |p| must be allowed in input, but lowercase should always be used in output.  A decimal point that is not immediately adjacent to a digit on both sides is prohibited.
...
The special infinity and NaN float values must not be allowed with any other capitalization.  An error must be raised if they are not supported by the float implementation.
...
The different varieties of unquoted strings are defined below.  As with integers and floats, these patterns are applied after any relevant line unwrapping has been performed (\cref{sec:indentation}).
\begin{grammar}
reserved_word = ("n" | "N") ("o" | "O") ("n" | "N") ("e" | "E") |
                ("t" | "T") ("r" | "R") ("u" | "U") ("e" | "E") |
                ("f" | "F") ("a" | "A") ("l" | "L") ("s" | "S") ("e" | "E") |
                ("i" | "I") ("n" | "N") ("f" | "F") |
                ("n" | "N") ("a" | "A") ("n" | "N")
hangul_filler = "\u115F" | "\u1160" | "\u3164" | "\uFFA0"
xid_start = unicode_xid_start_code_point - hangul_filler
xid_continue = unicode_xid_continue_code_point - hangul_filler
unquoted_ascii_identifier = "_"* ascii_alpha (ascii_alpha | dec_digit | "_")*
unquoted_unicode_identifier = "_"* xid_start xid_continue*
unquoted_ascii_sequence = unquoted_ascii_identifier
                          ("\u0020" (ascii_alpha | dec_digit | "_")+)*
unquoted_unicode_sequence = unquoted_xid_identifier
                            ("\u0020" xid_continue+)*
si_mu_prefix = "\u00B5" | "\u03BC" | "\U0001D707"
unquoted_ascci_dec_number_unit_string =
    (dec_integer | dec_float)
    ((ascii_alpha - "b" - "B" - "o" - "O" - "x" - "X" - "e" - "E" - "i" - "I" - "l") |
     ("x" | "X") (ascii_alpha - "a".."f" - "A".."F") ascii_alpha* |
     (ascii_alpha - "x" - "X" - "O" - "l") ascii_alpha+ |
     "%")
     
unquoted_unicode_dec_number_unit_string =
    unquoted_ascci_dec_number_unit_string |
    (dec_integer | dec_float) si_mu_prefix ascii_alpha+
unquoted_ascii_dict_key = unquoted_ascci_dec_number_unit_string |
                          unquoted_ascii_identifier - reserved_word
unquoted_unicode_dict_key_string = unquoted_unicode_dec_number_unit_string |
                                   unquoted_unicode_identifier - reserved_word
unquoted_ascii_general_string = unquoted_ascci_dec_number_unit_string |
                                unquoted_ascii_sequence - reserved_word
unquoted_unicode_general_string = unquoted_unicode_dec_number_unit_string |
                                  unquoted_unicode_sequence - reserved_word
\end{grammar}

Unquoted strings come in ASCII and Unicode variants.  By default, the ASCII variants must be used.  An implementation must provide an |unquoted_unicode| option to enable the Unicode variants.  However, the Unicode variants are not used by default because even the relatively restricted range of Unicode code points with the properties |XID_Start| and |XID_Continue| include a number of homoglyphs that may be confused with the ASCII punctuation characters that have special meaning in \bespon.  When |unquoted_unicode| is enabled, special care is required to counter the additional potential for security issues.

There are three categories of unquoted strings.  Unquoted strings may meet an identifier pattern, or be an integer or float followed immediately by a limited range of characters that can represent a unit.  These are the only unquoted strings that are allowed as keys in dict or dict-like collection types.  There are also general unquoted strings, which may be used everywhere else.  Unlike the previous categories, these may contain multiple space-separated ``words.''

In all cases, the set of valid unquoted strings excludes the reserved words (|none|, |true|, |false|, |inf|, |nan|) and all capitalization variants thereof.

Implementations must provide an |unquoted_strings| option that completely disables unquoted strings.


\paragraph{Quoted escaped strings}

\begin{grammar}
non_ignorable_valid_code_point = valid_code_point - unicode_default_ignorable_code_point
general_escape = "\\x" (lower_hex_digit | upper_hex_digit){2} |
                 "\\u" (lower_hex_digit | upper_hex_digit){4} |
                 "\\U" (lower_hex_digit | upper_hex_digit){8} |
                 "\\u" "{" (lower_hex_digit | upper_hex_digit){1,6} "}"
short_escape =
    "\\" ("\\" | "'" | '"' | "a" | "b" | "e" | "f" | "n" | "r" | "t" | "v" |
          "/" | space* "\n")
escape = general_escape | short_escape
empty_string = "''" | '""' | "``"
basic_inline_quoted_string =
    "'" (valid_code_point - "\\" - "\n" - "'" | escape)*
        (non_ignorable_valid_code_point - "\\" - "\n" - "'" | escape)+
        (valid_code_point - "\\" - "\n" - "'" | escape)* "'" |
    '"' (valid_code_point - "\\" - "\n" - '"' | escape)*
        (non_ignorable_valid_code_point - "\\" - "\n" - '"' | escape)+
        (valid_code_point - "\\" - "\n" - '"' | escape)* '"' |
...
\begin{grammar}
common_indent = indent*
string_indent = common_indent indent*
block_string =
    line_start
        (common_indent preceding_content | string_indent) "'"{n: n>2} space* "\n"
        ((string_indent ((valid_code_point - "\\" - "\n" | escape)* - "'"{n}) |
          indent*) "\n")+
        string_indent "'"{n} "/" |
    line_start
        (common_indent preceding_content | string_indent) '"'{n: n>2} space* "\n"
...
As with integers, floats, and unquoted strings, the inline (non-block) string patterns are applied after any relevant line unwrapping has been performed (\cref{sec:indentation}).  That is, inline strings may be wrapped onto multiple lines, provided that the indentation meets the unwrapping requirements.  The wrapped syntax this may be summarized as follows.
\begin{grammar}
common_indent = indent*
wrapped_inline_string = common_indent preceding_content* string_delim string_body_1
                        (common_indent indent* string_body_n "\n")*
                        common_indent indent* string_body_last string_delim 
\end{grammar}
Note that any wrapped line cannot consist solely of Unicode whitespace and |Default_Ignorable_Code_Point|s.  In unwrapping, |\n| is converted to a space if not preceded by Unicode whitespace, and removed otherwise.

Quoted strings using the single and double quotation marks U+0027 (|'|) and U+0022 (|"|) allow backslash escapes to represent Unicode code points.  By default, all Unicode code points may be represented by backslash escapes, with the exception of the surrogate code points.  Backslash escapes use the hex notations |\xHH| (8-bit), |\uHHHH| (16-bit), and |\UHHHHHHHH| (32-bit), as well as |\u{H...H}| (hex value consisting of 1--6 characters).  All of these must be allowed in loading.  The |\u{H...H}| notation should be preferred in dumping.

Common short escapes are also defined:  |\\|, |\'|, |\"|, |\a|, |\b|, |\e|, |\f|, |\n|, |\r|, |\t|, |\v|.  The escape |\/| should be allowed for cases when compatibility with JSON strings is desired.  The escape |\|\vmeta{space*}|\n| is replaced with the empty string, so that a string containing no whitespace may be wrapped across multiple lines.

The strings delimited with backticks U+0060 (|`|) are literal strings.  Backlash escapes are not allowed.  In literal strings delimited by |```| or a longer series of backticks, anything may occur except for the delimiter sequence (or, in inline strings, a sequence consisting solely of whitespace and |Default_Ignorable_Code_Point|s).  In inline literal strings, if the initial non-whitespace/final non-whitespace character is a backtick |`|, then the first/last character is stripped if it is a space.  This allows |``` ` ```| to represent the single backtick |`| with no leading or trailing spaces.

\textcolor{red}{\bfseries REVISE AND REORGANIZE}
...
\item Three or more adjacent quotation marks |"""{1,n}| represent the beginning of a string in which any sequence of quotation marks |"| of a different length than the delimiters may appear literally, without escaping.\footnote{Requiring that the delimiters be longer than any internal sequence of quotation marks would have had the advantage that quoting a \bespon\ string including its delimiters would be as simple as adding longer delimiters.  However, this would also have the severe disadvantage of requiring extremely long delimiters for a string that contains a long sequence of quotation marks but no quotation marks otherwise.}  The string ends at the next sequence of quotation marks that is the same length as the opening sequence.  The treatment of newlines and leading whitespace depends on the ending delimiter.
    ...
    \item If the string ends with |"""{1,n}/|, the opening and closing delimiters must be on lines separate from the actual string content, and those lines do not contribute to the string content.  All newlines are preserved, and a newline character or characters besides |\n| may be specified via explicit typing (\cref{sec:advanced-options-and-types:options}).  All lines must be indented to at least the level of the closing delimiter, except for lines consisting solely of whitespace; all indentation in common with the closing delimiter is stripped.  The opening delimiter must be at the same indentation level as the closing delimiter unless it is not at the beginning of the line, in which case indentation rules apply (\cref{sec:indentation}).
...
A base16 string is an explicitly typed Unicode string that matches the regular expression |[0-9a-f]*| or |[0-9A-F]*| once whitespace is stripped.  It is treated as a hexadecimal encoding of a byte string.  As usual, hex data may not mix capitalizations.
...
(list)>
\end{Verbatim}
does not represent a valid empty list.  Likewise,
\begin{Verbatim}
*
\end{Verbatim}
does not indicate an empty list.  The sequence
\begin{Verbatim}
[]
\end{Verbatim}
...
[1, 2, 3]
\end{Verbatim}
Trailing commas are allowed.  Whitespace between elements is discarded.


\subsection{Dictionary (\texttt{dict})}
\label{sec:type:dict}

A dictionary is an associative array that maps objects with scalar types to objects with arbitrary types.  Keys must be unique; duplicate keys are not allowed.

Dictionary keys must be scalar types by default, since these will have the best cross-language support and since these will generally be immutable types.  When possible, it is recommended that only Unicode strings be used as keys.  Some languages perform type coercion on some non-string keys.  For example, Python treats |True|, |1|, and |1.0| as identical keys; |False|, |0|, and |0.0| are also treated as identical.  In addition, using floats as keys can be problematic due to rounding errors.

Additional restrictions are placed on unquoted strings that are used as dict keys, regardless of any built-in or custom typing applied to the strings.  These cannot contain spaces, as other unquoted strings can.
...
key = value
\end{Verbatim}
If the value is on any lines other than the line on which the key ends, it must be indented relative to the key, except that a quoted string may have the same indentation as the key if the opening delimiter is on the same line as the end of the key.
...
\end{Verbatim}
Trailing commas are allowed and whitespace between elements is discarded.

Key can be explicitly typed, but cannot have advanced types (|alias|, |copy|, and |deepcopy|; \cref{sec:advanced-options-and-types:types}).  Only scalar types are allowed.  Quoted keys can used the |newline| advanced type option (\cref{sec:advanced-options-and-types:options}).  Keys cannot use the |label| advanced type option (\cref{sec:advanced-options-and-types:options}), to maintain symmetry with advanced types not being allowed.


\section{Extended types}

Core types must be supported unless a programming language lacks them.  Implementations are encouraged to support extended types, but this is not required.  By default, extended types require the use of an explicit type declaration unless specifically noted otherwise.  An implementation must raise an error when an unsupported extended type is encountered.
...
Decimal floating point number, which represents decimal numbers exactly.  The |decimal| type may be applied to numbers that would otherwise be floats or ints.  Since the typing is explicit, there is no need to force a decimal point followed by a zero (|.0|) for integer-valued decimals; this is only necessary in the |float| case because implicit typing would otherwise make it impossible to distinguish integer-valued floats from ints.  As in the |int| and |float| cases, a value that would cause an overflow must result in an error.


\subsubsection{Rational (\texttt{rational})}

Rational number.  The |rational| type applies to two integers separated by a solidus U+002F (|/|), with optional indentation around the solidus.


\subsubsection{Complex (\texttt{complex})}

Complex number.  An implicitly typed complex number consists of a float that is immediately followed by an |i| that represents the imaginary part, along with a second optional float representing the real part.  An explicitly typed |complex| may be applied to integer values as well, and only requires either the real or imaginary part (rather than always requiring the imaginary part as in the implicitly typed case).  Precision is not specified, but a 64-bit float is expected for both the real and the imaginary parts.  As in the |int| and |float| cases, a value that would cause an overflow must result in an error.


\subsubsection{Arbitrary precision integer (\texttt{bigint})}

This type must use at least the decimal part of the |int| pattern.  Using the full |int| pattern is encouraged when practical.


\subsubsection{Arbitrary precision float (\texttt{bigfloat})}

This type must use the combined decimal parts of the |float| and |int| patterns.  Using the full patterns is encouraged when practical.
...
When creating time data, the full available fractional second precision must be used by default.  When loading time data, if the second precision exceeds the capacity of the available time implementation, an error must be raised.  Implementations should provide an option to truncate, not round, the second data instead of raising an error.
...
Comments use the number sign U+0023 (|#|).  There are three forms of comments, some of which closely follow the syntax for quoted strings.
...
key = [value]
\end{Verbatim}
Spaces are allowed before and after \vmeta{decimal integer}, but this is discouraged.
...
The underscore U+005F (|_|) is special when it is the first element in a keypath.  Due to the definition of unquoted strings, it can never appear by itself elsewhere without quoting.  It refers to one level up from the last location.  For example,
\begin{Verbatim}
key.subkey.* = value1
_.* = value2
\end{Verbatim}
...
The tilde (slash) U+007E (|~|) is not normally allowed unquoted.  It is used in keypath aliases to denote the root data level.  In these situations, it is only allowed as the first element in a keypath.

The caret U+005E (|^|) is not normally allowed unquoted.  It is used in keypath aliases to refer to higher relative levels in the data structure.  It is only allowed as the first element in a keypath (|^.key|, |^^.key|, etc.).  Each additional |^| in a sequence indicates going up in the data structure by another level.
...
A section ends at the next section.  A section may optionally be ended by using |==={1,n}/|, with the opening and ending sequence of |=| having the same length.  If the closing delimiter |==={1,n}/| must be used either not at all or for every single section.
...
\end{Verbatim}
All option keys must be unquoted strings meeting the regex |[a-zA-Z][a-zA-Z0-9_]*|.  Values must be boolean or ASCII strings (quoted or unquoted, but no block strings).  When an option is given that is incompatible with a type, an error must be raised.  The \vmeta{type} may be omitted for implicitly typed objects.
...
Aliases are only resolved once the aliased object is resolved, with the exception that circular aliases may be resolved ``simultaneously'' with their targets.  If this is impossible, then an implementation must raise an error.  For example, suppose that an implementation adds support for an immutable |tuple| collection type, as exists in Python.  Then this could not be resolved:
\begin{Verbatim}
...
When this option is enabled, only a limited set of additional code points is allowed unquoted.  The set is based on \href{http://unicode.org/reports/tr31/}{Unicode Standard Annex \#31, Unicode Identifier and Pattern Syntax}.
...
\textcolor{red}{\bfseries Remove use of colon namespaces?}
...
\end{Verbatim}
Type names are never quoted, so they must match an identifier-style regex to avoid any potential ambiguity with implicitly typed objects.  In addition to matching the regex, type names must not match regexes and associated patterns for all types that are implicitly typed, including reserved words.  Libraries are encouraged to use type names of the form \vmeta{lib\_name}|::|\vmeta{type} for library-specific types that are not defined in the \bespon\ specification and whose interpretation is not immediately obvious.  
