...
[+-]? [\x20\t]* 0x _?
   (?: (?: [0-9a-f](?:_?[0-9a-f])*
           (?: \.[0-9a-f](?:_?[0-9a-f])* (?: _? [pP][+-]?[0-9](?:_?[0-9])* )? |
                    (?: _? [pP][+-]?[0-9](?:_?[0-9])* )
           )
       ) |
       (?: [0-9A-F](?:_?[0-9A-F])*
...
The special float values |[+-]?[\x20\t]*inf| and |[+-]?[\x20\t]*nan| are allowed.  An error must be raised if any of these appear with a different capitalization, or if any of these appear and are not supported by the specified float type.
