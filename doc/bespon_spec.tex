...
YAML has block literals that begin with \verbpipe.  That's an improvement until you need text with leading indentation or trailing empty lines.  Then you have to resort to things like this (visible space characters):
...
key2: ...
\end{Verbatim}
The |6| after the \verbpipe\ indicates that the text is indented by six spaces \emph{relative to the indentation} of |key1|, and the |+| means that trailing empty lines should be kept rather than discarded.  The result is text with literal newlines with an overall indentation of two spaces but a first line indented by four spaces, and with a trailing empty line.
...
\item[Strict keywords]  If unquoted strings are allowed, the potential for ambiguity between keywords and strings increases.  Keywords should only be allowed to appear in a form with accepted capitalization; appearance in any other form should trigger an error.  YAML's |True| is boolean true, but |TRue| is the string |TRue|, should be impossible.
...
\item[A small list of special characters] In YAML, almost all ASCII punctuation characters have a special meaning, which is context-dependent in many cases.  For example, |#| begins a comment, but only if preceded by a whitespace character.  The number of special characters should be kept to an easy-to-remember minimum, and as far as possible, they should not be context-dependent.  
...
Data files should be encoded with UTF-8, but choosing and correctly using an encoding is ultimately the user's responsibility.  In the absence of a specified encoding, UTF-8 must always be assumed for data in binary form.

In loading data, an implementation is only responsible for what it does with the data it receives.  When an implementation receives a Unicode string or stream, the implementation is not responsible for any transformations performed on the string or stream during any decoding or normalization steps before it was received.  When an implementation receives a binary string or stream, it must decode the string or stream as UTF-8 unless an encoding is specified.
...
\paragraph{Disallowed code points}  The string or stream must be checked for code points that are not allowed.  These code points may be represented in escaped form, but must not appear literally.  These fall into several categories.  The presence of any of these code points must result in an error.
\begin{itemize}
...
\end{itemize}
The list of disallowed code points is current as of Unicode 9.0 (01-Jun-2016 draft).  Barring unforseen circumstances, the list will be expanded only under two conditions:  (1) additional newline code points are defined, or (2) additional code points are added (or existing code points are identified) that pose a security concern similar to that of the bidi overrides.
...
\item[Newlines:]  One or more line feed U+000A (|\n|).
...
\bespon\ defines core types that must be defined in every implementation unless the implementation language lacks a comparable type.

If a language lacks an integer type, then by default it must interpret all integers as floats when loading data.  Implementations should provide an option to raise an error instead.  Such a language must only dump floats by default.  Implementations should provide a mechanism for the user to specify that some numbers are to be dumped as integers instead.  When \bespon\ is used as an interchange format between languages, and not all languages have integers, then it is recommended that all numbers be sent as floats, with explicit post-processing to integers as necessary.  A parser directive (\cref{sec:parser-directive}) may be used to enforce that all numbers are floats.
...
Many types, such as integers, floats, and booleans, are implicitly typed.  All types are also given a type name for use in explicit typing.  The syntax for explicit typing is
\begin{Verbatim}[commandchars=\\\{\}]
(\meta{type name})> \meta{quoted or unquoted string}
\end{Verbatim}
For example, |(float)> 1.23e4|.  In the discussion of types below, the type name for explicit typing is given in parentheses after the description of each type.

The parentheses |()| are special characters reserved for use in explicit typing, and may only appear unquoted for that use.  The greater-than sign |>|, which terminates the type declaration, is only special when it immediately follows a closing parenthesis |)|.  A |>| that terminates a type declaration must be separated by whitespace from any following |>|.


\subsection{None (null, nil, undef) (\texttt{none})}
...
Floats are defined by the following regular explressions (whitespace insensitive).  Equivalent regexes may be substituted.
...
By default, unquoted strings are allowed if (1) they contain only printable ASCII characters (U+0020 -- U+007E), (2) they do not include the special characters |%=,*'"{}[]()|, (3) they do not meet the pattern for any of the types described so far (|none|, |bool|, |int|, |float|).  When unquoted strings are used as dictionary keys, they must meet additional requirements (\cref{sec:type:dict}).

An unquoted string ends at the next character not allowed in an unquoted string, at the next line consisting only of whitespace, or at the next line with an indentation level that does not match that of the string, whichever is found first.  An unquoted string may contain |\n| to wrap it onto multiple lines.  When this is done, the string is unwrapped by stripping |\n| when preceded by Unicode whitespace and replacing |\n| with a space otherwise.
...
\subsection{Byte string (\texttt{b})}

\subsection{Base16 (hexadecimal) (\texttt{b16})}

\subsection{Base64 (\texttt{b64})}
...
\section{Explicit typing and additional types}

\subsection{Decimal (\texttt{decimal})}

\subsection{Rational (\texttt{rational})}

\subsection{Complex (\texttt{complex})}

\subsection{Arbitrary precision integer (\texttt{bigint})}

\subsection{Arbitrary precision float (\texttt{bigfloat})}

\subsection{Ordered dictionary (\texttt{odict})}
...
\section{Labels, references, copying, and overwriting}
