...
Rational number.  The |rational| type must use the |int| regex, followed by optional whitespace, followed by |/|, followed by optional whitespace, followed by the |int| regex.
...
An inline comment starts with at least three percent signs, |%%%{1,n}| and continues until the same number is encountered.  An inline comment cannot contain any sequence of |%| longer than the delimiters, or any lines consisting solely of whitespace.  It may begin and end on the same line.
...
A multiline comment begins with |%%%{1,n}| and continues until |%%%{1,n}/| is found.  It may contain anything except for a sequence of |%| of the same length or longer than the delimiters.  The opening delimiter must only be followed by whitespace on its line, and the closing delimiter must only be preceded by whitespace on its line.
...
\section{Advanced options and types:  newlines, labels, overwriting, references, copying}
\label{sec:advanced-options}

All advanced options and types should be included in an implementation, but may be omitted due to the added complexity.  When an unsupported advanced option or type is encountered, an error must be raised.
...
\end{Verbatim}
All option keys must be unquoted strings meeting the regex |[a-zA-Z][a-zA-Z0-9_-]*|.  Values must be |none|, |true|, |false|, or quoted or unquoted Unicode strings (but no strings with literal indentation and newlines).  Several option are defined by default.  When an option is given that is incompatible with a type, an error must be raised.

\paragraph{newline}
This specifies a character or sequence of characters that will be used to replace all literal newlines (|\n|) in a string.  Only Unicode newline code points/sequences from the Unicode Newline Guidelines (\href{http://unicode.org/standard/reports/tr13/tr13-5.html}{Unicode Technical Report \#13}) are allowed.  The newline replacement must be given as an escaped string.
...
\end{Verbatim}
Defining type names beginning with ``bespon'' is not allowed; these are reserved for potential future use in \bespon\ derivatives.
...
An implementation must use the default values for |unquoted_strings|, |unquoted_unicode|, and |only_ascii| unless other values are specified.  A parser directive can never turn |unquoted_strings| or |unquoted_unicode| on when the implementation has it off, or turn |only_ascii| off when the implementation has it on.  A parser directive must only restrict parsing relative to what the implementation allows with current settings; a parser directive must never cause less strict parsing than what the implementation allows with current settings.  Doing so could have security implications.  Any parser directive that conflicts with current implementation settings must result in an error.
