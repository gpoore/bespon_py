...
\item[Explicit typing]  JSON and TOML have no support for special or user-defined types unless a convention is agreed upon or unless an implementation provides features beyond the specification.
...
Once data is in the form of a Unicode string or stream, by default it must be validated according to the following procedure before processing.  Implementations may choose to validate all at once or a chunk at a time immediately before processing.  An implementation may provide an option that allows a less strict validation (allows some or all disallowed code points to appear as literals), but this must not be the default.
