...
\gdef\grammar@activate{%
  \expandafter\def\expandafter\@noligs\expandafter{%
    ...
    \def'####1'{\textcolor{DarkBlue}{%
      \textquotesingle\booltrue{grammar@instring}%
      \grammar@process@string####1\grammar@undefined%
      \boolfalse{grammar@instring}\textquotesingle}%
      \expandafter\@ifnextchar\grammar@charlb{\grammar@string@repeat}{}}%
    \def"####1"{\textcolor{DarkBlue}{%
      \detokenize{"}\booltrue{grammar@instring}%
      \grammar@process@string####1\grammar@undefined
      \boolfalse{grammar@instring}\detokenize{"}}%
      \expandafter\@ifnextchar\grammar@charlb{\grammar@string@repeat}{}}%
...
\begin{grammar}
unicode_code_points = "\u0000".."\U0010FFFF"
unicode_category_cc = "\u0000".."\u001F" | "\u007F".."\u009F"
unicode_newline_codepoints = "\u000A".."\u000D" | "\u0085" | "\u2028" | "\u2029"
unicode_property_bidi_control = "\u061C" | "\u200E".."\u200F" | 
                                "\u202A".."\u202E" | "\u2066".."\u2069"
unicode_surrogates = "\uD800".."\uDFFF"
bom = "\uFEFF"
unicode_noncharacters = "\uFDD0".."\uFDEF" | "\uFFFE" | "\uFFFF" |
                        "\U0001FFFE" | "\U0001FFFF" |
                        "\U0002FFFE" | "\U0002FFFF" |
                        "\U0003FFFE" | "\U0003FFFF" |
                        "\U0004FFFE" | "\U0004FFFF" |
                        ...
                        "\U000FFFFE" | "\U000FFFFF" |
                        "\U0010FFFE" | "\U0010FFFF"
valid_code_points = unicode_code_points
                     - (unicode_category_cc - "\t" - "\n")
                     - (unicode_newline_codepoints - "\n")
                     - unicode_property_bidi_control
                     - unicode_surrogates
                     - bom
                     - unicode_noncharacters
validated_string_or_stream = valid_code_points+
\end{grammar}
\end{grammarbox}
...
spaces = space+
tab = "\x09" = "\t"
indentation_code_points = space | tab
indentation = indentation_code_points+
newline = "\x0A" = "\n"
whitespace = (indentation_code_points | newline)+
...
\paragraph{Decimal}
\begin{grammar}
dec_integer = (sign space*)? (zero | nonzero_dec_digit ("_"? dec_digit)*) 
\end{grammar}

\paragraph{Hexadecimal}
\begin{grammar}
hex_integer = (sign space*)? "0x" "_"?
              (lower_hex_digits_underscores | upper_hex_digits_underscores)
\end{grammar}

\paragraph{Octal}
\begin{grammar}
oct_integer = (sign space*)? "0o" "_"? oct_digits_underscores
\end{grammar}

\paragraph{Binary}
\begin{grammar}
bin_integer = (sign space*)? "0b" "_"? bin_digits_underscores
\end{grammar}
...
dec_float = (sign space*)? (zero | nonzero_dec_digit ("_"? digit)*)
...
\begin{grammar}
infinity = (sign space*)? "inf"
not_a_number = (sign space*)? "nan"
\end{grammar}
...
\begin{grammar}
reserved_words = ("n" | "N") ("o" | "O") ("n" | "N") ("e" | "E") |
                 ("t" | "T") ("r" | "R") ("u" | "U") ("e" | "E") |
                 ("f" | "F") ("a" | "A") ("l" | "L") ("s" | "S") ("e" | "E")
xid_start = unicode_xid_start_code_point
xid_continue = unicode_xid_continue_code_point
xid_medial = "-"
unquoted_xid_identifier = "_"* xid_start (xid_medial? xid_continue)*
unquoted_xid_sequence = unquoted_xid_identifier ("\u0020" ("-" | xid_continue)+)*
si_mu_prefix = "\u00B5" | "\u03BC" | "\U0001D707"
unquoted_dec_number_unit_string =
    (dec_integer | dec_float)
    ((ascii_alpha - "b" - "B" - "o" - "O" - "x" - "X" - "e" - "E" - "i" - "I" - "l") |
     ("x" | "X") (ascii_alpha - "a".."f" - "A".."F") ascii_alpha* |
     (ascii_alpha - "x" - "X" - "O" - "l") ascii_alpha+ |
     si_mu_prefix ascii_alpha+)
unquoted_dict_key_string = unquoted_dec_number_unit_string |
                           unquoted_xid_identifier - reserved_words
unquoted_string = (unquoted_dec_number_unit_string |
                   (dec_integer | dec_float) "\u0020" unquoted_xid_sequence |
                   unquoted_xid_sequence - reserved_words)
\end{grammar}
...
\begin{enumerate}
\item Start := XID\_Start, plus all code points in U+0021 through U+007E except for the special characters |%=,*'"{}[]()| and the characters checked for at the beginning of an invalid digit |+-.0123456789|, minus the Hangul fillers (U+115F, U+1160, U+3164, U+FFA0).
\item Continue := XID\_Continue, plus all code points in U+0021 through U+007E except for the special characters |%=,*'"{}[]()|, minus the Hangul fillers (U+115F, U+1160, U+3164, U+FFA0), plus space.
\end{enumerate}
