...
\paragraph{Disallowed code points}  The string or stream must be checked for code points that are not allowed to appear literally.  These code points may only be represented in escaped form.  The presence of any of these code points must result in an error.
\begin{itemize}
\item All code points with Unicode General\_Category value Control (Cc)\footnote{Control includes the ranges U+0000 through U+001F, and U+007F through U+009F (\href{http://www.unicode.org/versions/Unicode9.0.0/ch02.pdf}{The Unicode Standard, Version 9.0, Chapter 2.4}).} are prohibited, with the exception of line feed U+000A (|\n|) and horizontal tab U+0009 (|\t|).  Note that any carriage returns U+000D (|\r|) not removed by newline normalization are not allowed.  These code points are prohibited because they may be invisible in some editors and because some control characters, such as the null byte, have security implications.

\item All 7 code points treated as newlines (\href{http://www.unicode.org/versions/Unicode9.0.0/ch05.pdf}{The Unicode Standard, Version 9.0, Chapter 5.8}) are prohibited, with the exception of line feed U+000A (|\n|).  This includes vertical tab U+000B (|\v|), form feed U+000C (|\f|), carriage returns U+000D (|\r|), and next line U+0085 (NEL), which are already covered under the Cc category.  It also includes line separator U+2028 and paragraph separator U+2029.  These characters are prohibited to ensure that newline treatment is uniform and to guarantee that the use of any newlines beyond line feed U+000A (|\n|) is explicit.

\item All 12 code points with Unicode property \href{http://unicode.org/reports/tr9/}{Bidi\_Control} are prohibited: U+061C, U+200E through U+200F, U+202A through U+202E, and U+2066 through U+2069.  Bidi override characters can be a security concern in general, and are particularly problematic in a configuration format designed for manual editing.  

\href{http://unicode.org/reports/tr36/}{Unicode Technical Report \#36}, ``Unicode Security Considerations,'' recommends that overrides never be allowed in IRIs.  However, even the implicit directional marks can have a potent effect when operating on punctuation characters.  For example, the sequence
\begin{Verbatim}
{"a"="b\u200f", $=!, "\u200fe"="f"}
\end{Verbatim}
...
\end{itemize}
The list of prohibited literal code points is current as of Unicode 9.0.0 (2016-06-01).  Given the \href{http://unicode.org/policies/stability_policy.html}{Unicode stability policies} and the maturity of Unicode, it seems unlikely that the list will need modification as a result of future Unicode versions.  The list of prohibited code points will be expanded if additional code points are identified or are assigned that pose a security concern similar to that raised by the bidi controls.
